...
function obtenerDiasSeleccionados() {
    return Array.from(camposDias)
        .filter((campoDia) => campoDia.checked)
        .map((campoDia) => {
            const etiqueta = document.querySelector(`label[for="${campoDia.id}"]`);
...
function guardarRegistro() {
    ...
    const nuevoRegistro = {
        ...
        actividadNombre: campos.actividad_nombre.value.trim(),
        descripcion: campos.descripcion.value.trim(),
        dias: obtenerDiasSeleccionados(),
        horaInicio: campos.hora_inicio.value,
        horaFin: campos.hora_fin.value,
        archivoNombre: campos.archivo.files[0].name,
