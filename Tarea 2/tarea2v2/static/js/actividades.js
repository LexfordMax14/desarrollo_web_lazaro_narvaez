...
function crearHtmlActividad(indice) {
    return `
        <div class="actividad-grupo" data-actividad="${indice}">
            ...
            <label for="descripcion_${indice}">Descripcion:</label>
            ...
            <label for="dia_${indice}">Dia:</label>
            ...
            <label for="duracion_${indice}">Duracion (HH:MM):</label>
            ...
            <button type="button" class="btn-eliminar" onclick="eliminarActividad(${indice})" style="background-color: #d32f2f;">
...
function crearOpciones(opciones) {
    return opciones
        .map((opcion) => `<option value="${opcion}">${capitalizar(opcion)}</option>`)
        .join('');
}
