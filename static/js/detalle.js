...
// Reglas de validacion del comentario (deben coincidir con el servidor)
const NOMBRE_MIN = 3;
const NOMBRE_MAX = 80;
const TEXTO_MIN = 5;
...
// Datos basicos del miembro como lista de definiciones
...
// Una actividad: sus datos, sus fotos, sus comentarios y el formulario
function crearArticuloActividad(actividad, index) {
    ...
    // Carga asincrona de los comentarios existentes
    cargarComentarios(actividad.id, listaComentarios);

    return articulo;
}

// Pide los comentarios de una actividad y los pinta en la lista
async function cargarComentarios(actividadId, lista) {
    try {
        ...
        if (!result.success || result.data.length === 0) {
            ...
            vacio.textContent = "Aun no hay comentarios.";
...
// Un comentario: fecha, nombre y texto (textContent evita inyeccion de HTML)
...
// Formulario para agregar un comentario a una actividad
...
// Validacion en el cliente (mismas reglas que el servidor)
...
// Muestra mensajes de error manteniendo el formulario visible
...
// Valida en cliente, envia con fetch (POST) y refresca la lista
async function enviarComentario(actividadId, inputNombre, textarea, errores, listaComentarios, boton) {
    ...
    try {
        ...
        // Exito: limpiar formulario y recargar los comentarios
        mostrarErrores(errores, []);
        inputNombre.value = "";
        textarea.value = "";
        cargarComentarios(actividadId, listaComentarios);
    } catch (error) {
        mostrarErrores(errores, [`Error de conexion: ${error.message}`]);
    } finally {
