...
function validarDatosMiembro() {
    ...
    if (!/^9\d{8}$/.test(telefono)) {
        mostrarError('error_telefono', 'Teléfono inválido (debe ser 9XXXXXXXX)');
        esValido = false;
    }
    if (!comuna) {
...
function validarTodasLasActividades() {
    ...
    document.querySelectorAll('.actividad-grupo').forEach(grupo => {
        if (!validarActividad(grupo)) esValido = false;
    });
    return esValido;
}
...
function validarActividad(grupo) {
    ...
    if (!dia) {
        mostrarError('error_dia_' + indice, 'Debe seleccionar un día');
        esValido = false;
    }
    if (!horaInicio) {
    ...
    if (!duracion) {
        mostrarError('error_duracion_' + indice, 'Debe ingresar la duración');
        esValido = false;
    }
    if (archivo && !/\.(jpg|jpeg|png)$/i.test(archivo.name)) {
        mostrarError('error_archivo_' + indice, 'Extensión no permitida (jpg, jpeg, png)');
        esValido = false;
    }
